...
/**
 * Owner-only "delete this look set" control with an inline confirm step.
 * Mirrors DeleteReportButton. `onDeleted` lets a client list drop the row
 * optimistically instead of a full refresh; without it we fall back to
 * router.refresh() / redirectTo (used from the set-detail header).
 */
export function DeleteSetButton({
  ...
  async function remove() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/look-set/${setId}`, { method: "DELETE" });
      if (!res.ok) {
        const payload = (await res.json().catch(() => null)) as {
          error?: string;
        } | null;
        throw new Error(payload?.error ?? "Could not delete set");
      }
      if (onDeleted) {
        onDeleted(setId);
        return;
