-- Rule-based style tags for catalogue products.
--
-- Recommendation ranking used to lean almost entirely on the product title +
-- vector similarity, which is blind to items with weak lexical signals. These
-- structured tags (computed at ingest by scripts/feeds/tags.mjs) let the app
-- rank on data instead of substrings:
--   formality    1 (beach/sport) … 5 (full formal)  | null = not applicable
--   trend_level  0 (timeless) … 3 (loud, of-the-moment)
--   versatility  0 (hard to place) … 3 (works with almost anything)
-- A one-off backfill (scripts/backfill-product-tags.mjs) populates existing rows.

alter table public.products
  add column if not exists formality smallint,
  add column if not exists trend_level smallint,
  add column if not exists versatility smallint;

-- Extend the offer-aware search to return the style tags (and color_hex from
-- 0025) so the app can rank matches on them.
